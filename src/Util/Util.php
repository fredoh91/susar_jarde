...
class Util
{
    ...
    public static function CreeSUSAR(ManagerRegistry $doctrine, array $lstSusar, string $TypeSusar, string $importTypeSusarEu, string $lastUsername): void
    {
        $entityManager = $doctrine->getManager();

        foreach ($lstSusar as $susar_a_creer) {
            if ($entityManager->getRepository(Susar::class)->EstCe_SUSAR_unique($susar_a_creer['id'])) {
                if ($entityManager->getRepository(Susar::class)->EstCe_SUSAR_unique_specificcaseid_DLPVersion($susar_a_creer['specificcaseid'], $susar_a_creer['DLPVersion'])) {
                    ...
                    $Susar->setUtilisateurImport($lastUsername);
